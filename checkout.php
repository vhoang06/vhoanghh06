    ...
    if (empty($errors)) {
        try {
            ...
            $stmt = $pdo->prepare("
                ...
                VALUES (?, ?, ?, ?, ?, ?, 'cod')
            ");
            $stmt->execute([$_SESSION['user_id'], $total, $shipping_name, $shipping_phone, $shipping_address, $note]);
            ...
            foreach ($cart_items as $item) {
                // Kiểm tra tồn kho (khóa dòng để tránh race condition)
                $stmt_get_stock->execute([$item['id']]);
                $current_stock = (int)$stmt_get_stock->fetchColumn();
                if ($current_stock < $item['quantity']) {
                    throw new Exception("Sản phẩm '" . ($item['name'] ?? $item['id']) . "' không đủ tồn kho ({$current_stock} còn, yêu cầu {$item['quantity']})");
                }

                // Cập nhật tồn kho
                $stmt_update_stock->execute([$item['quantity'], $item['id']]);

                // Thêm vào order_items với giá tại thời điểm đặt
            ...
            if (!empty($_SESSION['checkout_items'])) {
                // Lấy danh sách ID sản phẩm đã checkout
                $checked_product_ids = [];
                foreach ($_SESSION['checkout_items'] as $item) {
                    $checked_product_ids[] = $item['id'];
                }
                
                // Lọc giỏ hàng để bỏ sản phẩm đã checkout
                $_SESSION['cart'] = array_values(array_filter($_SESSION['cart'], function($item) use ($checked_product_ids) {
                    return !in_array($item['id'], $checked_product_ids);
                }));
            } else {
                unset($_SESSION['cart']);
            }
            
            // Xóa session checkout_items
            ...
            $pdo->rollBack();
            $errors[] = "Có lỗi xảy ra khi xử lý đơn hàng: " . $e->getMessage();
        }
    }
}
?>
